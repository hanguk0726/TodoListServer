...
func (t *taskItemUseCase) SynchronizeTaskList(taskItem ...domain.TaskItem) {
	for _, v := range taskItem {
		if t.taskItemRepository.DoesExists(v) {
			t.taskItemRepository.UpdateTaskItem(v)
		} else {
			t.taskItemRepository.AddTaskItem(v)
		}
	}
}
